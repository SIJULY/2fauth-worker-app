...
export const vaultService = {
    ...
    async getCategories() {
        try {
            return await request('/api/vault/categories')
        } catch (e) {
            throw new vaultError('Failed to fetch categories', 'CATEGORIES_FETCH_FAILED', e)
        }
    },
    ...
    async updateSortOrder(items) {
        try {
            return await request('/api/vault/sort', {
                method: 'POST',
                body: JSON.stringify({ items })
            })
        } catch (e) {
            throw new vaultError('Failed to update sort order', 'SORT_UPDATE_FAILED', e)
        }
    },
