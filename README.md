...
> 💡 **提示**：演示站开启了全员登录模式，数据可意修改、删除。正式部署请务必设置白名单。
...
services:
  2fauth:
    ...
    environment:
      ...
      - OAUTH_GITHUB_REDIRECT_URI=https://你的域名/api/oauth/callback
...
2. 回调地址 (Callback URL) 必须填写：`https://你的域名/api/oauth/callback`。
...
| 平台 | Client ID 变量名 | Client Secret 变量名 | Redirect URI 回调变量名 (示例: `https://xxx.dev/api/oauth/callback`) |
| :--- | :--- | :--- | :--- |
