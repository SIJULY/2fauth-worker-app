...
if (checkTable['count(*)'] === 0) {
    ...
    // Check for schema updates (e.g., sort_order)
    try {
        const columns = sqlite.prepare("PRAGMA table_info(vault)").all() as any[];
        const hasSortOrder = columns.some(col => col.name === 'sort_order');
        if (!hasSortOrder) {
            console.log('Adding sort_order column to vault table...');
            sqlite.exec("ALTER TABLE vault ADD COLUMN sort_order INTEGER DEFAULT 0;");
            console.log('sort_order column added.');
        }
    } catch (e) {
        console.error('Failed to check/update schema:', e);
    }
}
...
serve({
    ...
    console.log(`[Docker Server] 2FAuth is running on http://0.0.0.0:${info.port}`);
});
