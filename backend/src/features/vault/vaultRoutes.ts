...
vault.get('/', async (c) => {
    ...
    const result = await service.getAccountsPaginated(page, limit, search, category, sortBy);
...
vault.post('/sort', async (c) => {
    ...
    const service = getService(c);
    const result = await service.updateSortOrder(items);
