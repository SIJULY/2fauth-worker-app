...
export class VaultService {
     ...
     * 获取分页和搜索条件后的所有账户 (解密)
     */
    async getAccountsPaginated(page: number, limit: number, search: string, category?: string, sortBy?: string) {
        const items = await this.repository.findPaginated(page, limit, search, category, sortBy);
     ...
     */
    async getCategories() {
        return await this.repository.findCategories();
    }
