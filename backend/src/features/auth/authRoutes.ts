...
auth.get('/authorize/:provider', async (c) => {
    ...
    const isSecure = new URL(c.req.url).protocol === 'https:' || c.req.header('x-forwarded-proto') === 'https';
...
auth.post('/callback/:provider', async (c) => {
    ...
    const isSecure = new URL(c.req.url).protocol === 'https:' || c.req.header('x-forwarded-proto') === 'https';
    deleteCookie(c, stateCookieName, { path: '/', secure: isSecure, sameSite: 'Lax' });
    // --- State 闭环校验结束 ---
...
auth.post('/logout', (c) => {
    const isSecure = new URL(c.req.url).protocol === 'https:' || c.req.header('x-forwarded-proto') === 'https';
    const cookieOpts = { path: '/', secure: isSecure, sameSite: 'Strict' as const };
